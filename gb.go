...
func drawBoxCorners(box Box) {
	ui.SetCell(box.TL.X, box.TL.Y, '┌', box.LineColor, d)
	ui.SetCell(box.TR.X, box.TR.Y, '┐', box.LineColor, d)
	ui.SetCell(box.BL.X, box.BL.Y, '└', box.LineColor, d)
	ui.SetCell(box.BR.X, box.BR.Y, '┘', box.LineColor, d)
}

func drawBoxSides(box Box) {
	for i := box.TL.X + 1; i < box.TR.X; i++ {
		ui.SetCell(i, box.TL.Y, '─', box.LineColor, d)
		ui.SetCell(i, box.BL.Y, '─', box.LineColor, d)
	}
	for j := box.TL.Y + 1; j < box.BL.Y; j++ {
		ui.SetCell(box.TL.X, j, '│', box.LineColor, d)
		ui.SetCell(box.BR.X, j, '│', box.LineColor, d)
	}
}
...
func main() {
	...
	box := makeBox("grimmwa.re", ridonk, 1, 2, h-3, w-3, orange)
	someMsg := "lots of words and stuff"
	box2 := makeBox("stuff", someMsg, 4, 5, 3, len(someMsg)+2, ui.ColorDefault)
